...
export default {
  ...
  props: {
    ...
    height: String,
    lockX: Boolean, // 是否禁止横向滚动
    lockY: Boolean, // 是否禁止竖向滚动
    scrollbarX: Boolean, // 是否显示横向滚动条
    scrollbarY: Boolean, // 是否显示竖向滚动条
    bounce: {
      type: Boolean,
      default: true
    },
  ...
  watch: {
    ...
    value: {
      handler: function(val) {
        if (
          val.pullupStatus === 'default' &&
          this.currentValue.pullupStatus !== 'default'
        ) {
          this.donePullup()
        }
        if (
          val.pulldownStatus === 'default' &&
          this.currentValue.pulldownStatus !== 'default'
        ) {
          this.donePulldown()
        }
        if (
          val.pullupStatus === 'disabled' &&
          this.currentValue.pullupStatus !== 'disabled'
        ) {
          this.disablePullup()
        }
        if (
          val.pullupStatus === 'enabled' &&
          this.currentValue.pullupStatus === 'disabled'
        ) {
          this.enablePullup()
        }
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      if (this.usePulldown) {
        ...
        const container = this.$slots.pulldown
      ...
      if (this.usePullup) {
        // if use slot=pullup
        const container = this.$slots.pullup
